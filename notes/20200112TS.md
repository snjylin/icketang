...
在类体中可以定义三类数据：
	第一种自身属性数据：
		可以通过constructor构造函数定义自身属性或者是方法
	这类数据会被每一个实例化对象所访问
...
第二种原型数据：
	我们直接在类体中定义原型方法即可
	但是，如果要定义原型属性数据，则必须要使用get、set设置特性的方式来定义
		get 取值器
		set 赋值器
	由于对数据设置了特性，在查看对象的时候，这些数据将直接展示出来
```
// ES6中定义类的方式
class People {
	// 通过constructor构造函数定义自身数据
...
暴露接口是通过export
引入模块的方式有三种：
	1 import * as 模块 from 模块路径
		实现将所有的接口共同引入
	2 import { data } from 模块路径
		实现从模块中解构出某几个接口
	3 import 模块 from 模块路径
		实现引入默认的接口
暴露接口：
	1 通过export向外暴露
	2 通过export default 暴露默认接口
高级写法：
	可以通过逗号，实现同时引入默认接口以及解构出来的接口
...
# 下午复习：
	...
	语法：
		class 类名 {}
	在类中可以定义三种数据类型：
		1 自身数据 通过constructor构造函数来定义
		2 原型数据
			直接在类体中定义方法即可  原型属性数据 要通过设置特性的方式
		3 静态数据  
			在类体中，在数据的前面使用static关键字即可  在类的外部直接添加
	继承：
		class child extends father {}
		将会得到父类中所有的数据
		我们还可以重写继承过来的属性和方法，此时子类将优先使用重写的数据
		我们还可以通过constructor重写构造函数，但是必须传递super关键字实现继承

	Es Module：已经纳入es7中
	引入模块的方式：
		1 import * as 模块 from 模块文件
		2 import { data } from 模块文件
		3 import Data from 模块文件
	暴露接口：
		1 export 暴露接口
		2 export default 暴露默认接口
	引入css样式文件：
		直接引入样式文件
			通过import 样式文件
	TS: ts是js的超集
	数据类型： 在ts中为所有的数据指定类型
	类型猜测： 如果定义的数据没有指明类型，此时将会进行类型猜测
	类型推断： 语法 1 <type>数据  2 数据 as 类型
	数组：	   let arr:type[] = []
	元组：	   let arr:[type1, type2] = []
...
在ts中要为每一个函数指明类型
语法：
	function demo(arg:type, arg1?:type):type {}
传递参数：
	1 传递的数据类型要一致
	2 传递的数据个数要一致
注意：
...
语法：
	funciton demo<T>(arg: T):T {}
		此时，返回值就是一致的，都是T变量表示的类型
...
我们还可以重写构造函数，还可以重写继承过来的属性以及方法，此时将会优先使用重写的数据
	必须传递super关键字，还要传递必要的参数
	子类中特有的属性要定义在super关键字之下
